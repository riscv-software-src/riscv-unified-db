...
module Udb
  ...
  class Eqn
    ...
    tt_path = Pathname.new(__FILE__)
    rb_path = Pathname.new(__dir__) / "eqn_parser.rb"
    ...
    if needs_grammar_recompile
      ...
      rb_src = tt_compiler.ruby_source_from_string(EQN_GRAMMAR)

      # make sure there is a single newline at the end
      File.write rb_path, "#{rb_src.strip}\n"
    end
    require_relative "eqn_parser"
