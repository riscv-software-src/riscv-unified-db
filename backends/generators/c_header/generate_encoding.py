...
def load_resolved_exception_codes(resolved_codes_file):
    """Load pre-resolved exception codes from JSON file generated by Ruby ERB processing."""
    exception_codes = []

    try:
        with open(resolved_codes_file) as f:
        ...
        logging.info(
            f"Loaded {len(exception_codes)} pre-resolved exception codes from Ruby ERB processing"
        )
        return exception_codes

    except Exception as e:
        logging.error(
            f"Error loading resolved exception codes from {resolved_codes_file}: {e}"
        )
        return []
...
def main():
    ...
    parser.add_argument(
        "--resolved-codes",
        help="JSON file containing pre-resolved exception codes from Ruby ERB processing",
    )
    ...
    # Load exception codes from pre-resolved Ruby ERB processing
    if args.resolved_codes:
        logging.info(f"Loading pre-resolved exception codes from {args.resolved_codes}")
        causes = load_resolved_exception_codes(args.resolved_codes)
    else:
        logging.error(
            "No resolved exception codes file provided. Use --resolved-codes parameter."
        )
        causes = []
