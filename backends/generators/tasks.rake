...
namespace :gen do
  ...
  task sverilog: "#{$root}/gen/sverilog" do
    ...
    # Run the SystemVerilog generator script using the same Python environment
      sh "#{$root}/.home/.venv/bin/python3 #{$root}/backends/generators/sverilog/sverilog_generator.py --inst-dir=#{inst_dir} --csr-dir=#{csr_dir} --ext-dir=#{ext_dir} --resolved-codes=#{resolved_codes_file.path} --output=#{output_dir}riscv_decode_package.svh --include-all"
    ensure
      resolved_codes_file.close
      resolved_codes_file.unlink
    end
