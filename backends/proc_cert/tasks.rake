...
def proc_cert_create_adoc(erb_template_pname, target_pname, model_name, normative_rules, extra_bindings = nil)
  ...
  unless normative_rules.nil?
    raise ArgumentError, "normative_rules must be a NormativeRules class but is a #{normative_rules.class}" unless normative_rules.is_a?(Udb::NormativeRules)
  end
  ...
  proc_cert_design = Udb::ProcCertDesign.new(model_name, cfg_arch, Udb::ProcCertDesign.proc_ctp_type, proc_cert_model_with_cfg_arch,
    proc_cert_model_with_cfg_arch.proc_cert_class, normative_rules)
