...
template <unsigned N> struct BitsStorageType {
...
template <unsigned N> struct BitsSignedStorageType {
  using type = std::conditional_t<
      (N > 128), mpz_class,
      std::conditional_t<
          (N > 64), __int128,
...
template <bool Signed> class _RuntimeBits;
...
// IDL.
template <unsigned N, bool Signed> class _Bits {
  static_assert(N > 0);
...
public:
  ...
  // value of N that represents unknown precision (happens when there is a left
  // shift by unknown value)
  ...
  // beyond this, the number of bits is still tracked, but the storage is using
  // gmp
  ...
  template <unsigned _N = N> static consteval bool needs_mask() {
...
public:
  ...
  template <unsigned... _N> struct constmax {
  ...
  // saturating add
  template <unsigned A, unsigned B> struct addsat {
...
public:
  ...
  static constexpr SignedStorageType
  cast_to_signed(const StorageType &unsigned_value) {
    if constexpr (N == (sizeof(StorageType) * 8)) {
      ...
      // we have a native type, but some bits are unsed. need to sign extend the
      // storage
  ...
  template <bool _Signed> _Bits(const _RuntimeBits<_Signed> &);
  ...
  template <class IntType>
    requires(!std::is_same_v<StorageType, IntType> && std::integral<IntType> &&
             needs_mask())
  ...
  constexpr _Bits<constmax<N, M>::value, Signed && _Signed> operator op(       \
    ...
    if constexpr (M > N) {                                                     \
      return _Bits<constmax<N, M>::value, Signed && _Signed>{                  \
          _Bits<M, _Signed>{get()}.get() op o.get()};                          \
    } else {                                                                   \
      return _Bits<constmax<N, M>::value, Signed && _Signed>{                  \
          get() op _Bits{o.get()}.get()};                                      \
    }                                                                          \
  }                                                                            \
                                                                               \
...
#define BITS_OP_ASSIGN(op)                                                     \
  template <typename T> constexpr _Bits &operator op##=(const T & o) {         \
...
#undef BITS_OP_ASSIGN
  ...
  // We want to be able to use it as a template parameter because templated IDL
  // functions will do so.
  StorageType m_val;
};

// used to translate literal strings into a Bits<N> type
template <char... Str> struct BitsStr {
    ...
    if (base == 10) {
      ...
      for (int i = len - 1; i >= 0; i--) {
        ...
        if (val < last_val) {
          ...
          // trying to find the width from the decimal string at compile time is
          // tricky
...
//
// auto b = -15_b; // b will be +1, because 15_b is only four bits, and negation
// loses the sign bit
template <char... Str>
...
template <unsigned N, bool Signed> class numeric_limits<udb::_Bits<N, Signed>> {
public:
...
namespace udb {
template <unsigned N> using Bits = _Bits<N, false>;

template <unsigned N> using SignedBits = _Bits<N, true>;
...
public:
  ...
  template <typename T> _RuntimeBits operator<<(const T &shamt) {
...
template <unsigned N, bool Signed> class _PossiblyUnknownBits {
public:
