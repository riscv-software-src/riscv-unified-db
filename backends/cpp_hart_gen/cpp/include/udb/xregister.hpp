...
template <unsigned XLEN> class XRegister {
public:
  ...
  template <unsigned N> operator Bits<N>() const { return m_value; }
  ...
  template <typename T>
  decltype(std::declval<ValueType>().sra(std::declval<T>()))
  sra(const T &shamt) const {
