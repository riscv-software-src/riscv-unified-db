  ...
  class ConfigValidator {
    ...
    static nlohmann::json validate(const YAML::Node& config) {
      ...
      if (schema == "config-0.1.0.json") {
        ...
        try {
          validator.set_root_schema(nlohmann::json::parse(DbData::SCHEMAS[schema]));
        } catch (const std::exception * e) {
          throw std::runtime_error("Validation of schema config-0.1.0 failed: " + std::string(e->what()));
        }
        ...
        try {
          ...
          throw std::runtime_error("Config validation failed: " + std::string(e.what()));
        }
      }
      return json;
    }
