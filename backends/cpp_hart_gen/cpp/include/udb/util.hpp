...
constexpr Bits<size>
extract(const BitfieldMember<BitfieldParentSize, BitfieldMemberStart,
...
template <unsigned P, unsigned N, unsigned M, typename StartType,
          typename SizeType>
Bits<BitfieldMember<P, N, M>::Width>
extract(const BitfieldMember<P, N, M> &value, const StartType &start,
        const SizeType &size) {
...
template <typename... BitsTypes> struct ConcatWidth;
...
template <> struct ConcatWidth<> {
  static constexpr unsigned Width = 0;
};

template <typename BitsType, typename... BitsTypes>
constexpr Bits<ConcatWidth<BitsType, BitsTypes...>::Width>
__concat(const BitsType &a, const BitsTypes &...bits) {
