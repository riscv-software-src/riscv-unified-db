...
def compare_yaml_json_encoding(instr_name, yaml_match, yaml_vars, json_encoding_str, repo_dir):
    ...
    # Decide expected instruction length: 16 for compressed, else 32
    expected_length = 16 if instr_name.lower().startswith(('c_', 'c.')) else 32

    # Turn YAML's '-' into '.' for variable bits
    yaml_pattern_str = yaml_match.replace('-', '.')

    # Check if YAML pattern length matches
    ...
    # Resolve variables & track their locations
    yaml_var_positions = {}
    ...
    # Regex that captures single bits ('0','1','-') or multi-char tokens
    tokens = re.findall(r'[01]|-|[A-Za-z0-9]+(?:\[\d+\]|\[\?\])?', json_encoding_str)

    # Build bit-index from highest to lowest
    json_bits = []
    ...
    # Compare each bit: fixed '0'/'1' vs. variable
