...
rule %r{#{$root}/gen/.*/pdf/.*_extension\.pdf} => proc { |tname|
  ...
  [
    "#{$root}/gen/#{config_name}/adoc/#{ext_name}_extension.adoc"
  ]
} do |t|
  ...
  adoc_file = "#{$root}/gen/#{config_name}/adoc/#{ext_name}_extension.adoc"

  FileUtils.mkdir_p File.dirname(t.name)
  File.write t.name, Asciidoctor.convert_file(adoc_file, backend: "pdf", safe: :safe)
end

rule %r{#{$root}/gen/.*/adoc/.*_extension\.adoc} => proc { |tname|
  config_name = Pathname.new(tname).relative_path_from($root / "gen").to_s.split("/")[0]
  [
    "#{$root}/.stamps/arch-gen-#{config_name}.stamp",
    ($root / "views" / "adoc" / "ext_pdf.adoc.erb").to_s,
    __FILE__
  ]
} do |t|
  config_name = Pathname.new(t.name).relative_path_from("#{$root}/gen").to_s.split("/")[0]
  ...
  template_path = $root / "views" / "adoc" / "ext_pdf.adoc.erb"
...
namespace :gen do
  ...
  task :ext_pdf, [:extension, :config_name] do |_t, args|
    ...
    Rake::Task[$root / "gen" / config.to_s / "pdf" / "#{extension}_extension.pdf"].invoke
  end
end
