# vqdotus.vx.yaml - unsigned vs2, signed xs1
@"
...
description: |
  Vector quad widening unsigned-signed dot product instruction performing the dot product between a 4-element vector of 8-bit unsigned integer elements (vs2) and a scalar 4-element vector of 8-bit signed integer elements (xs1), accumulating the result into a 32-bit signed integer accumulator.
  ...
  Where vs2[i] is a 32-bit bundle containing four 8-bit unsigned integers and xs1 contains four 8-bit signed integers in its lower 32 bits.
definedBy: Zvqdotq
...
operation(): |
  # Vector quad widening unsigned-signed dot product (vector-scalar)
  # SEW must be 32, vs2 is unsigned, xs1 is signed
  ...
  for (i = 0; i < vl; i++) {
    if (vm[i] || vm == 1) {  # Check mask
      # Extract 4 8-bit unsigned elements from vector bundle
      vs2_elem0 = unsigned_byte(vs2[i][7:0]);
      vs2_elem1 = unsigned_byte(vs2[i][15:8]);
      vs2_elem2 = unsigned_byte(vs2[i][23:16]);
      vs2_elem3 = unsigned_byte(vs2[i][31:24]);
      
      # Compute dot product: unsigned * signed
...
"@ | Out-File -FilePath "spec\std\isa\inst\Zvqdotq\vqdotus.vx.yaml" -Encoding UTF8
