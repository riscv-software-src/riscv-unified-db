...
stored in `tools/tests/regress-tests.yaml`.

To add a basic regression, you must:

1. Add the regression in `tools/tests/regress-test.yaml`
...
2. run `./bin/ruby tools/test/gen_regress.rb`
--
...
1. add the steps as a job in `tools/tests/regress-gh-template`
2. run `./bin/ruby tools/test/gen_regress.rb`
--
