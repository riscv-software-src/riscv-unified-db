...
THe hand-written (unresolved) specification files can use either, but not both, condition forms in a given place.
...
There are three types of terms in UDB conditions:
...
* an *xlen requirement*
** For example, `xlen` equals '32' +
   (`xlen` references the runtime xlen in the current mode, which may be different than `MXLEN`)
...
Note that RISC-V does not follow semantic versioning, so `2.0` may be compatible with `1.0` for a given extension as long as `2.0` (or any version between `1.0` and `2.0`) is not marked as a breaking version when it was defined.
See <<versioning.adoc#>> for more information on breaking changes.
...
[source,yaml]
----
# require D, version _compatible with_ 1.0.0
extension:
  name: D
  version: ~> 1.0
----
...
[source,yaml]
----
# require D, version greater than or equal to 1.0.0 and less than 2.0.0
extension:
  name: D
  version: [">= 1.0", "< 2.0"]
----
...
It expects a parameter `name`, and a test.

When the parameter is a scalar, the test must be one of the following:

[cols="1m,3d,2m"]
|===
| Key                   | Meaning                                     | Example

| equal                 | Parameter value equals
a|
...
| notEqual              | Parameter value is not equal to
a|
...
| lessThan             | Parameter value is less than                        | lessThan: 5
| greaterThan          | Parameter value is greater than                     | greaterThan: 5
| lessThanOrEqual      | Parameter value is less than or equal to            | lessThanOrEqual: 5
| greaterThanOrEqual   | Parameter value is greater than or equal to         | greaterThanOrEqual: 5
| oneOf                | Parameter value is equal to one element of an array | oneOf: [5, 7]
|===

When the parameter is an array, the test can either check a specific element, check for inclusion,
or compare the overall size of the array.

[cols="1m,3d,2m"]
|===
| Key                   | Meaning                                       | Example

| index                 | Perform scalar comparison on element at index +
                          Must be paired with a scalar comparison.      | index: 1 +
                                                                          equal: 5
| includes              | Array parameter includes a value
a|
...
| size                  | Use the array size as the comparison value +
                          Must be paired with a scalar comparison       | size: true +
                                                                          equal: 2
|===
...
Notably, if/else statements are not permitted. Implications should be used instead.
...
* `xlen()`
** Runtime XLEN in the current mode (32 or 64)
