# vqdotsu.vx.yaml - signed vs2, unsigned xs1
@"
...
operation(): |
  # Vector quad widening signed-unsigned dot product (vector-scalar)
  # SEW must be 32, vs2 is signed, xs1 is unsigned
  
  if (SEW != 32) {
    raise(ExceptionCode::IllegalInstruction, mode(), encoding);
  }
  
  # Extract 4 8-bit unsigned elements from scalar register (lower 32 bits)
  xs1_elem0 = unsigned_byte(xs1[7:0]);
  xs1_elem1 = unsigned_byte(xs1[15:8]);
  xs1_elem2 = unsigned_byte(xs1[23:16]);
  xs1_elem3 = unsigned_byte(xs1[31:24]);
  ...
  for (i = 0; i < vl; i++) {
    if (vm[i] || vm == 1) {  # Check mask
      ...
      # Compute dot product: signed * unsigned
...
"@ | Out-File -FilePath "spec\std\isa\inst\Zvqdotq\vqdotsu.vx.yaml" -Encoding UTF8
