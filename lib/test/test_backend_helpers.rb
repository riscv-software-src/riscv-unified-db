...
class TestAsciidocUtils < Minitest::Test
  def test_resolve_links_ext
    assert_equal("xref:#ext-foo-def[bar]", AsciidocUtils.resolve_links("%%LINK%ext;foo;bar%%"))
    assert_equal("xref:#ext-foo-def[foo]", AsciidocUtils.resolve_links(link_to_ext("foo")))
  end

  def test_resolve_links_ext_param
    assert_equal("xref:#ext_param-foo-bar-def[zort]", AsciidocUtils.resolve_links("%%LINK%ext_param;foo.bar;zort%%"))
    assert_equal("xref:#ext_param-foo-bar-def[bar]", AsciidocUtils.resolve_links(link_to_ext_param("foo","bar")))
  end

  def test_resolve_links_inst
    assert_equal("xref:#inst-foo-def[bar]", AsciidocUtils.resolve_links("%%LINK%inst;foo;bar%%"))
    assert_equal("xref:#inst-foo-def[foo]", AsciidocUtils.resolve_links(link_to_inst("foo")))
  end

  def test_resolve_links_csr
    assert_equal("xref:#csr-foo-def[bar]", AsciidocUtils.resolve_links("%%LINK%csr;foo;bar%%"))
    assert_equal("xref:#csr-foo-def[foo]", AsciidocUtils.resolve_links(link_to_csr("foo")))
  end

  def test_resolve_links_csr_field
    assert_equal("xref:#csr_field-foo-bar-def[zort]", AsciidocUtils.resolve_links("%%LINK%csr_field;foo.bar;zort%%"))
    assert_equal("xref:#csr_field-foo-bar-def[foo.bar]", AsciidocUtils.resolve_links(link_to_csr_field("foo","bar")))
  end
