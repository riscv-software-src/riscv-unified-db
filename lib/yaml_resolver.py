...
def merge_file(rel_path : str | Path, arch_dir : str | Path, overlay_dir : str | Path | None, merge_dir : str | Path) -> None:
  ...
  if not os.path.exists(arch_path) and (overlay_path == None or not os.path.exists(overlay_path)):
    ...
    if not os.path.exists(merge_path) or (os.path.getmtime(overlay_path) > os.path.getmtime(merge_path)):
      shutil.copyfile(os.path.join(overlay_dir, rel_path), merge_path)
  else:
    # both exist, merge
    if not os.path.exists(merge_path) or (os.path.getmtime(overlay_path) > os.path.getmtime(merge_path)) or (os.path.getmtime(arch_path) > os.path.getmtime(merge_path)):
      arch_obj = read_yaml(os.path.join(arch_dir, rel_path))
      overlay_obj = read_yaml(os.path.join(overlay_dir, rel_path))

      write_yaml(os.path.join(merge_dir, rel_path), json_merge_patch(arch_obj, overlay_obj))
