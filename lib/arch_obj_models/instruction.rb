...
class Instruction < DatabaseObject
  ...
  def cert_coverage_points
    ...
    @data["cert-coverage-points"]&.each do |cert_coverage_point|
      @cert_coverage_points << CertCoveragePoint.new(cert_coverage_point)
    end
    @cert_coverage_points
  end
  ...
  class CertCoveragePoint
    ...
    # @return [Array<CertLink>] Sorted list of certification point links (cross references)
    def cert_links
  ...
  class CertLink
    ...
    # For sorting ...
    def <=>(other)
      if other.is_a?(CertLink)
        @data <=> other.data
      else
        raise ArgumentError, "CertLink is not comparable to a #{other.class.name}"
      end
    end
  end
end
