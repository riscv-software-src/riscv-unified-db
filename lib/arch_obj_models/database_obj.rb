...
class DatabaseObject
 ...
 # Exception raised when there is a problem with a schema file
  ...
  # @param data [Hash<String,Object>] Hash with fields to be added
  # @param data_path [Pathname] Path to the data file
  # @param arch [Architecture] The RISC-V standard (can optionally be a ConfiguredArchitecture too)
  def initialize(data, data_path, arch)
    raise ArgumentError, "data is a #{data.class} but needs to be a Hash" unless data.is_a?(Hash)
    raise ArgumentError, "data_path is a #{data_path.class} but needs to be a Pathname" unless data_path.is_a?(Pathname)
    raise ArgumentError, "arch is a #{arch.class} but needs to be an Architecture" unless arch.is_a?(Architecture)
