...
class Idl::AstNode
  def gen_adoc(_indent = 0, indent_spaces: 2)
...
module Idl
  ...
  class AryRangeAssignmentAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      "#{' ' * indent}#{variable.gen_adoc(indent,
                                          indent_spaces:)}[#{msb.gen_adoc(0,
                                                                          indent_spaces:)}:#{lsb.gen_adoc(0,
                                                                                                          indent_spaces:)}] = #{write_value.gen_adoc(
                                                                                                            0, indent_spaces:
                                                                                                          )}"
    end
  end
  ...
  class ConditionalReturnStatementAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      "#{' ' * indent}#{return_expression.gen_adoc(indent,
                                                   indent_spaces:)} if (#{condition.gen_adoc(0, indent_spaces:)});"
    end
  end
  ...
  class MultiVariableAssignmentAst
    def gen_adoc(indent, indent_spaces: 2)
      "#{' ' * indent}(#{variables.map do |v|
        v.gen_adoc(0, indent_spaces:)
      end.join(', ')} = #{function_call.gen_adoc(0, indent_spaces:)})"
    end
  end
  ...
  class FieldAssignmentAst
    def gen_adoc(_indent, indent_spaces: 2)
  ...
  class MultiVariableDeclarationAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      "#{' ' * indent}#{type_name.gen_adoc(0, indent_spaces:)} #{var_name_nodes.map do |var|
        var.gen_adoc(0, indent_spaces:)
      end.join(', ')}"
    end
  end
  ...
  class TernaryOperatorExpressionAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      "#{' ' * indent}#{condition.gen_adoc(0,
                                           indent_spaces:)} ? #{true_expression.gen_adoc(0,
                                                                                         indent_spaces:)} : #{false_expression.gen_adoc(
                                                                                           0, indent_spaces:
                                                                                         )}"
    end
  end
  ...
  class ForLoopAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      lines = ["#{' ' * indent}for pass:[(]#{init.gen_adoc(0,
                                                           indent_spaces:)}; #{condition.gen_adoc(0,
                                                                                                  indent_spaces:)}; #{update.gen_adoc(
                                                                                                    0, indent_spaces:
                                                                                                  )}) {"]
      stmts.each do |s|
  ...
  class VariableDeclarationWithInitializationAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      if ary_size.nil?
        "#{' ' * indent}#{type_name.gen_adoc(0,
                                             indent_spaces:)} #{lhs.gen_adoc(0,
                                                                             indent_spaces:)} = #{rhs.gen_adoc(0,
                                                                                                               indent_spaces:)}"
      else
        "#{' ' * indent}#{type_name.gen_adoc(0,
                                             indent_spaces:)} #{lhs.gen_adoc(0,
                                                                             indent_spaces:)}[#{ary_size.gen_adoc(0,
                                                                                                                  indent_spaces:)}] = #{rhs.gen_adoc(
                                                                                                                    0, indent_spaces:
                                                                                                                  )}"
      end
    end
  ...
  class AryElementAssignmentAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      "#{' ' * indent}#{lhs.gen_adoc(0,
                                     indent_spaces:)}[#{idx.gen_adoc(0,
                                                                     indent_spaces:)}] = #{rhs.gen_adoc(0,
                                                                                                        indent_spaces:)}"
    end
  end
  ...
  class FunctionCallExpressionAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      after_name = []
      unless template_arg_nodes.empty?
        after_name << "<#{template_arg_nodes.map do |t|
          t.gen_adoc(0, indent_spaces:)
        end.join(', ')}>"
      end
  ...
  class CsrFieldReadExpressionAst
    def gen_adoc(indent = 0, indent_spaces: 2)
      ...
      if idx_text =~ /[0-9]+/
        "#{' ' * indent}#{csr_text}"
      elsif @cfg_arch.csr(csr_text).nil?
        "#{' ' * indent}#{csr_text}"
      else
