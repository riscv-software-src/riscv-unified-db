# Fix vqdotu.vv.yaml - unsigned version
$content = @"
...
operation(): |
  ...
  for (i = 0; i < vl; i++) {
    if (vm[i] || vm == 1) {  # Check mask
      ...
      vs1_elem0 = unsigned_byte(vs1[i][7:0]);
      vs1_elem1 = unsigned_byte(vs1[i][15:8]);
      vs1_elem2 = unsigned_byte(vs1[i][23:16]);
      vs1_elem3 = unsigned_byte(vs1[i][31:24]);
...
"@; Set-Content "spec\std\isa\inst\Zvqdotq\vqdotu.vv.yaml" -Value $content
