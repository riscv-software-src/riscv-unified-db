# vqdotsu.vv.yaml - signed vs2, unsigned vs1
@"
...
operation(): |
  ...
  for (i = 0; i < vl; i++) {
    if (vm[i] || vm == 1) {  # Check mask
      # Extract 4 8-bit elements: vs2 signed, vs1 unsigned
      ...
      dot_product = vs2_elem0 * vs1_elem0 + 
                    vs2_elem1 * vs1_elem1 + 
                    vs2_elem2 * vs1_elem2 + 
                    vs2_elem3 * vs1_elem3;
...
"@ | Out-File -FilePath "spec\std\isa\inst\Zvqdotq\vqdotsu.vv.yaml" -Encoding UTF8
